...
///
/// ⚠ **왜 앱 타깃이 아니라 라이브러리인가.** `Sources/Ghostbar/` 는 executable 이라
/// `swift test` 가 못 부른다 — `GhostbarCore`(#8)·`GhostbarEmbedding`(#31)·
/// `GhostbarIndex`(#32) 를 뽑은 것과 **똑같은 이유**다. 걷으면 무슨 일이 다시 일어나나:
...
public enum CloudDraftWire {
    ...
    ///
    /// ⚠ **스트리밍을 안 쓴다.** 조각 뽑기는 한 번에 받아 통째로 뜯는 쪽이 단순하고,
    /// 화면은 어차피 다 온 뒤에야 카드를 그린다(`onFragmentDraft`). 상류가 남긴
    /// `StreamDelegate`/`StreamParsers` 는 **채팅 답을 토큰마다 화면에 흘리는** 물건이라
    /// 여기 쓸 자리가 없다 — 억지로 끼우면 델리게이트가 WKWebView 를 물게 된다.
