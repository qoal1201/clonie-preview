...
class WKWebViewWrapper: NSObject, WKScriptMessageHandler, WKNavigationDelegate {
    ...
    init(frame: NSRect = .zero) {
        ...
        view = WKWebView(frame: frame, configuration: configuration)
        super.init()
    ...
    func userContentController(_ ucc: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        case "saveBackend":
            ...
            // ★ **판정은 `ClonieCloud.BackendKeyRule` 이 든다** (#65 ③ 잔여). 여기 있던 사다리는
            ...
            //   (`tests/ClonieCloudTests/BackendKeyRuleTests.swift`). 여기 남는 것은 **읽고 넘기기**뿐이다.
            guard let b = message.body as? [String: Any] else { return }
    ...
    /// 실제로 묻는 자리는 그 기능을 켤 때다(`InterviewEars`).
    ///
    ...
    /// One request shape per provider, one response path back to the settings screen.
    func fetchModels(requestID: Int? = nil) {
        let configuration = BackendConfig.current
        let ollama = configuration.type == .ollama
        let anthropic = configuration.type == .anthropic
        var base = configuration.url.trimmingCharacters(in: .whitespacesAndNewlines)
        if base.isEmpty && anthropic { base = "https://api.anthropic.com/v1" }
        while base.hasSuffix("/") { base.removeLast() }
        guard let endpoint = URL(string: base + (ollama ? "/api/tags" : "/models")),
        ...
        } else if !ollama && !configuration.apiKey.isEmpty {
        ...
        URLSession.shared.dataTask(with: request) { [weak self] bytes, response, error in
            ...
            guard error == nil,
                  ...
                  let rows = object[ollama ? "models" : "data"] as? [[String: Any]] else {
                self.sendModelsState("error", message: "모델 목록을 불러오지 못했어요", requestID: requestID)
                return
            }
            var names = rows.compactMap { $0[ollama ? "name" : "id"] as? String }
            if !ollama { names.sort() }
            guard let encoded = try? JSONSerialization.data(withJSONObject: names),
                  let list = String(data: encoded, encoding: .utf8) else { return }
            let family = ollama ? "ollama" : "openai"
            let identifier = requestID.map(String.init) ?? "null"
            DispatchQueue.main.async { [weak self] in
                self?.view.evaluateJavaScript("receiveModels(\(list),\(jsLiteral(family)),\(identifier))",
                                              completionHandler: nil)
            }
        }.resume()
    }
...
///
/// 따옴표만 치환하면 줄바꿈·제어문자가 포함된 본문에서 깨진다.
