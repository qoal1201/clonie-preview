...
/// ## 쓰는 법
///
/// 유효한 `CLONIE_QA_SESSION=example`로 QA 앱을 실행한 경우:
///
///     echo '{"id":"t1","js":"document.querySelectorAll(\".card\").length"}' \
///       > /tmp/clonie-qa-example/drive.json
///     notifyutil -p com.local.clonie.qa.drive.example
///     cat /tmp/clonie-qa-example/drive-result-t1.json
///     cat /tmp/clonie-qa-example/qa-drive.log
///
