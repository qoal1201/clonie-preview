...
public enum EmbeddingModelStore {
    ...
    /// 실제로 볼 자리들.
    ///
    /// ⚠ **`CLONIE_MODEL_DIR` 이 있으면 거기만 본다 — 기본 자리로 안 물러선다.**
    ...
    public static func status(
        ...
        status(environment: environment, bundleResourcesURL: bundleResourcesURL, supportRoot: nil)
    }

    /// `supportRoot`는 실제 사용자 파일 대신 임시 Application Support를 시험할 때 주입한다.
    public static func status(
        environment: [String: String] = ProcessInfo.processInfo.environment,
        bundleResourcesURL: URL? = Bundle.main.resourceURL,
        supportRoot: URL?
    ...
    private static func supportModelDirectory(named product: String, under root: URL) -> URL {
