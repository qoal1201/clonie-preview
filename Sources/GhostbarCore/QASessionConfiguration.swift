...
public struct QASessionConfiguration: Equatable, Sendable {
    ...
    public var outputDirectory: String { "/tmp/ghostbar-qa-\(id)" }
    public func signal(_ action: String) -> String { "com.local.ghostbar.qa.\(action).\(id)" }

    public enum ConfigurationError: Error { case invalidSession, invalidVault }

    public init(environment: [String: String]) throws {
        guard let id = environment["GHOSTBAR_QA_SESSION"], !id.isEmpty, id.count <= 64,
        ...
        guard let path = environment["GHOSTBAR_QA_VAULT"], path.hasPrefix("/"), path != "/",
