...
public final class FragmentStore {
    ...
    /// `~/Library/Application Support/Clonie/cue.json`.
    ///
    /// ⚠ 폴더 이름이 아직 `Clonie` 다 — 가제(Cue)가 굳으면 레포명·번들 ID 와 **한 번에** 옮긴다.
    /// 여기서 혼자 바꾸면 사용자 파일이 뒤에 남는다.
    public static func defaultURL(fileManager: FileManager = .default) throws -> URL {
        let base = try fileManager.url(for: .applicationSupportDirectory,
                                       in: .userDomainMask, appropriateFor: nil, create: true)
        return base.appendingPathComponent("Clonie", isDirectory: true)
