...
/// → 환경변수 `GHOSTBAR_QA_VISIBLE=1` 일 때만 `.readOnly` 로 연다.
///
...
/// `open Ghostbar.app` 은 환경변수를 안 넘기므로, QA 로 켜려면 알맹이를 직접 부른다:
///
///     GHOSTBAR_QA_VISIBLE=1 ./Ghostbar.app/Contents/MacOS/Ghostbar &
///
...
enum WindowPrivacy {
    ...
    static let qaVisibleEnvVar = "GHOSTBAR_QA_VISIBLE"
